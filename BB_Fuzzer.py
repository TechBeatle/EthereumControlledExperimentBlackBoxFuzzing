...
def updateStorageAccessMap_D(txHashlist, txmap, tx_hash, RunGas, VulnerabilitiesList, ReEntrancy):
...
def updateStorageAccessMap_N(txHashlist, txmap, tx_Order, tx_E_Order, RunGas, VulnerabilitiesList, ReEntrancy):
    tx_Ex_Order = []
    for t in tx_E_Order:
        if t in tx_Order:
...
def BB_Fuzzer_D(F1, F3, fRecord, contract, addressArray, Q, priority, Q_size, txHashlist, VulnerabilitiesList, log_seek, ExtraTestCase):
    Rungas = 800000
    ...
    try:
        ...
        for f in seq:
            if F1 == 'M':
                account = [OwnerAccount, NormalAccount]
            if F1 == 'A':
            ...
            for a in account:
                tx = Transaction(-999, a, f.Name, value, inp_seq, 0, priority, None)
                SeqMap.append(tx)
    except Exception as e:
        print(e)
    ...
    for tx in SeqMap[:TotalTx]:
        try:
            ...
            log_seek, txmap, tx_Exec_Order = readGethLog(log_seek, txmap, tx_hash, tx_Exec_Order)
            txHashlist[tx_hash] = Transaction(-999, tx.From, tx.Function, tx.Value, tx.Input, 0, tx.Priority, tx_hash)
            updateStorageAccessMap_D(txHashlist, txmap, tx_hash, Rungas, VulnerabilitiesList, False)
        except Exception as e:
            print(e)
    ...
    return [Q, Q_size, txHashlist, VulList, log_seek, TotalTx]


def BB_Fuzzer_N(F1, F3, fRecord, contract, addressArray, Q, priority, Q_size, txHashlist, VulnerabilitiesList, log_seek, ExtraTestCase):
    Rungas = 800000
    ...
    for tx in SeqMap[:TotalTx]:
        try:
            ...
            txHashlist[tx_hash] = Transaction(-999, tx.From, tx.Function, tx.Value, tx.Input, 0, tx.Priority, tx_hash)
                ...
                print(e)
    # time.sleep(0.25)
    log_seek, txmap, tx_E_Order = readGethLog(log_seek, txmap, tx_hash, tx_Exec_Order)
    updateStorageAccessMap_N(txHashlist, txmap, tx_Order, tx_E_Order, Rungas, VulnerabilitiesList, False)
...
def setup(F2):
    ...
    while not answerreceived:
        ...
        if ans.lower() == "y":
            ...
            answerreceived = True
            try:
                print("Connected to chain " + w3.eth.chainId + " with latest block number:" + str(w3.eth.blockNumber))
                connected = True
                intermediatevariables.append(ans)
            except:
...
def MainLoop(F1, F2, F3):
                ...
                while retries < 3:
                    try:
                        ...
                        if F2 == 'D':
                            ...
                            BB_Fuzzer_D(F1, F3, fRecord, contract, addressArray, Q, priority, Q_size,txHashlist,
                                        VulnerabilitiesList, log_seek, ExtraTestCase)
                        if F2 == 'N':
                            [Q, Q_size, txHashlist, VulnerabilitiesList, log_seek, Totaltx] = \
                            BB_Fuzzer_N(F1, F3, fRecord, contract, addressArray, Q, priority, Q_size, txHashlist,
                                        VulnerabilitiesList, log_seek, ExtraTestCase)
                        ...
                        DataWriter = pd.ExcelWriter('Results ('+F1+','+F2+','+F3+').xlsx')
    ...
    os.rename('Results ('+F1+','+F2+','+F3+').xlsx', 'Results ('+F1+','+F2+','+F3+') '+ str(time.ctime())[3:19] + '.xlsx')
